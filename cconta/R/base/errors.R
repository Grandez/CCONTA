JGGBaseCond = R6::R6Class("JGG.BASE.COND"
   ,portable  = FALSE
   ,cloneable = FALSE
   ,lock_class = TRUE
   ,public = list(
...
Warning = function(msg, action=NULL, subclass=NULL, ...) {
 ...
 SQLError = function(msg, ...) {
      data = list(...)
      msg = paste0(msg, " (", data$sqlcode, ")")
     .error(msg, subclass="SQL", ...)
}
HTTPError  = function(msg, ...) { .error(msg, subclass="HTTP",  ...) }
EXECError  = function(msg, ...) { .error(msg, subclass="EXEC",  ...) }
MODELError = function(msg, ...) { .error(msg, subclass="MODEL", ...) }
